  ...
  socket.on('message', function(data) {
    try {
      ...
      if (['LEAVE', 'CANDIDATE', 'OFFER', 'ANSWER'].indexOf(message.type) !== -1) {
        ...
        // Response is only sent to keep the connection alive
      } else {
...
PeerServer.prototype._initializeHTTP = function() {
  ...
  this._app.get(this._options.path + ':key/list', function(req, res, next) {
    var key = req.params.key;
    var clientIds = [];
    if (self._clients[key]) {
      clientIds = Object.keys(self._clients[key]);      
      // for (var i = 0; i < clientIds.length; i += 1) {        
      //   var destination = self._clients[key][clientIds[i]];
      
      //   if (destination) {
      //     try {            
      //       if (destination.socket) {              
      //         destination.socket.send(JSON.stringify({ type: 'data', payload: { msg: 'test'}, 
      //             src:  clientIds[i]}
      //         ));             
      //       }            
      //     } catch (e) {
      //       console.log(e);
      //     }      
      //     res.send("send command done: " + clientIds.length);
      //   }
      // }       
    }
    res.send(clientIds);    
  ...
  this._app.post(this._options.path + ':key/:id/:token/ping', function(req, res, next) {
    return next();
  });
